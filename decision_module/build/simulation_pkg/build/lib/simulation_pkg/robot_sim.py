...
class SoccerBall:
    def __init__(self, x: float, y: float):
        ...
        self.last_update = time.time()  # store the last update time
    ...
    def update(self, dt: float, robots: List['OmniwheelRobot']) -> None:
        ...
        # If possessed, follow the robot’s position and velocity
        ...
        # Update positions using the (possibly updated) velocities
        ...
        # Apply extra horizontal damping when nearly on the ground
        if self.state.z < 0.1:
            self.state.vx *= (1 - (1 - GROUND_DAMPING) * dt)
            self.state.vy *= (1 - (1 - GROUND_DAMPING) * dt)

        # --- Field boundaries (and robot possession test) ---
        if self.state.x < 0:
            self.state.x = 0
        ...
        # --- Check for robot possession or bounce ---
        if self.state.z < BALL_DIAMETER / 2:
            for i, robot in enumerate(robots):
                dx = self.state.x - robot.x
                dy = self.state.y - robot.y
                dist = math.sqrt(dx * dx + dy * dy)
                # Increase the catch radius if needed
                if dist < CATCH_RADIUS and self.state.possession_cooldown <= 0:
                    self.state.possessed_by = i
                    break
                elif dist < BOT_SIZE:
                    # Bounce off robot: a simple reflection (could be tuned)
...
class OmniwheelRobot:
    def __init__(self, robot_number: int, x: float, y: float, theta: float = 0, color: tuple = (255, 0, 0)):
        ...
        # Add new variables to store global target velocities
        self.global_target_vx = 0
        self.global_target_vy = 0
        self.global_target_omega = 0
        ...
        # Track target and actual wheel velocities
        self.target_velocities = [0, 0, 0, 0]  # Target velocities in ticks/second
        self.current_wheel_velocities = [0, 0, 0, 0]  # Actual velocities in ticks/second
        
        # Cumulative ticks (for display only)
        ...
        # Add timestamps for rate control
    ...
    def set_velocity(self, vx: float, vy: float, omega: float):
        ...
        scale = 1/math.sqrt(2)
        wheel_velocities = [
            (vx + vy)*scale + omega,  # FR
            (-vx + vy)*scale + omega,  # FL
            (-vx - vy)*scale + omega,  # BL
            (vx - vy)*scale + omega   # BR
        ]
        self.set_wheel_velocities(wheel_velocities)
    
    def set_wheel_velocities(self, velocities: List[float]):
        """Set target velocities for wheels in ticks/second"""
        self.target_velocities = [
            np.clip(v, -MAX_TICKS_PER_SECOND, MAX_TICKS_PER_SECOND) 
            for v in velocities
        ]
    
    def update_pid(self, current_time: float):
        if current_time - self.last_pid_update >= 1.0/PID_RATE:
            dt = current_time - self.last_pid_update
            
            # Convert global velocities to local frame using current orientation
            self.target_vx = (self.global_target_vx * math.cos(self.theta) + 
                            self.global_target_vy * math.sin(self.theta))
            self.target_vy = (-self.global_target_vx * math.sin(self.theta) + 
                            self.global_target_vy * math.cos(self.theta))
            self.target_omega = self.global_target_omega

            self.motor_forces = []
            for i in range(4):
                pid_output = self.pid_controllers[i].compute(
                    self.target_velocities[i],
                    self.current_wheel_velocities[i],
                    dt
                )
    ...
    def update_physics(self, current_time: float):
        if current_time - self.last_physics_update >= 1.0/PHYSICS_RATE:
            dt = current_time - self.last_physics_update
            
            # Calculate net forces and torque
            fx = fy = 0
            torque = 0
            ...
            self.world_vx = self.vx * math.cos(self.theta) - self.vy * math.sin(self.theta)
            self.world_vy = self.vx * math.sin(self.theta) + self.vy * math.cos(self.theta)
            
            self.x += self.world_vx * dt
            self.y += self.world_vy * dt
...
class ROS2RobotSimulation(Node):
    def __init__(self):
        ...
        # Create separate timers for physics, control, and rendering
        self.create_timer(1.0/RENDER_RATE, self.render)
        self.create_timer(1.0/PHYSICS_RATE, self.update_physics)
        self.create_timer(1.0/PID_RATE, self.update_control)
    ...
    def update_physics(self):
        ...
        for robot in self.robots:
            robot.update_physics(current_time)
        self.ball.update(dt, self.robots)
    ...
    def cmd_vel_callback(self, msg: Twist, robot_index: int):
        ...
        self.robots[robot_index].global_target_vx = msg.linear.x * TICKS_PER_METER
        self.robots[robot_index].global_target_vy = -msg.linear.y * TICKS_PER_METER
        self.robots[robot_index].global_target_omega = -msg.angular.z * WHEEL_DISTANCE * TICKS_PER_METER/100
    ...
    def publish_robot_state(self, robot: OmniwheelRobot, robot_index: int):
        ...
        state_msg.layout.dim = [MultiArrayDimension()]
        state_msg.layout.dim[0].label = "state"
        state_msg.layout.dim[0].size = 9  # x, y, theta, 4 encoder values
        state_msg.layout.dim[0].stride = 9
        
        # Fill data array [x, y, theta, encoder1, encoder2, encoder3, encoder4]
        state_msg.data = [
            (robot.x - FIELD_WIDTH/2)/100,                    # x position in m
            (FIELD_HEIGHT/2 - robot.y)/100,                    # y position in m
